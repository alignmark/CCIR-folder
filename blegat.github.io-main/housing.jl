...
@constraint(model, sum(d)==selected) # ensuring M locations are selected
...
@objective(model, Min, ) # we use the SAW method to combine all the distances with given preference weightings to form an objective function
